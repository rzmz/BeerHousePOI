...
public class PurchaseTab {
	...
	protected void cancelPurchaseButtonClicked() {
		...
		try {
			...
			endSale();
			PurchaseInfoTableModel cart = model.getCurrentPurchaseTableModel();
			StockTableModel wareHouse = model.getWarehouseTableModel();
			for (int i = 0; i < cart.getRowCount(); i++) {
				StockItem stockItem = wareHouse.getItemById((long) cart
						.getValueAt(i, 0));
				stockItem.setQuantity(stockItem.getQuantity()
						+ (int) cart.getValueAt(i, 3));
			}
	...
	protected void submitPurchaseButtonClicked() {
		...
		final List<SoldItem> itemsInCart = model.getCurrentPurchaseTableModel()
				.getTableRows();

		double amount = 0.0;

		for (SoldItem item : itemsInCart) {
			amount += item.getSum();
		}
		
		final ConfirmPaymentFrame confirmPaymentFrame = new ConfirmPaymentFrame(
				amount);
		...
		confirmPaymentFrame.getCancelButton().addActionListener(
				new ActionListener() {

					@Override
					public void actionPerformed(ActionEvent e) {
						log.debug("Got cancel from ConfirmPaymentFrame");
						confirmPaymentFrame.dispose();
					}
